...
def por_categoria(categoria):
    """Instituciones de la categoría, cada una con su `logo` ya resuelto."""
    return [dict(inst, logo=ruta_logo(inst["slug"]))
            for inst in INSTITUCIONES if inst["categoria"] == categoria]
